...
AccelStepper Cutter2        (AccelStepper::DRIVER,3,4);
#define CutterEn           6
...
void setup() 
{
  ...
  pinMode(CutterEn, OUTPUT);
  digitalWrite(CutterEn, LOW);
  ...
  Cutter2.setMaxSpeed(Speed_Cutter_Raise_Lower);
...
//    Cutter.runSpeedToPosition();
  ...
  else if (Val == "LOWERCUTTER2")
  {
    digitalWrite(CutterEn, HIGH);
    BGTask[17] = true;
  }
    else if (Val == "RAISECUTTER2")
  {
    digitalWrite(CutterEn, HIGH);
    BGTask[18] = true; 
  }
...
void lowerCutter()
{
  if (Cutter.currentPosition() == positions[5])//-200)
  {
    BGTask[6] = false;
...
void homeCutter2()
{
  if (IsCutterClose())
  {
    if (Debug)
    {
      Serial.println("Cutter2 is raised.");
    }
    BGTask[18] = false;
    Cutter2.setCurrentPosition(0);
    positions[5] = 0;
    Serial.println(95477);
    digitalWrite(CutterEn, LOW);
  }
  ...
  {
    positions[5] += 1;
    Cutter2.moveTo(positions[5]);
  }
}

void lowerCutter2()
{
  if (Cutter2.currentPosition() == -120)
  {
    BGTask[17] = false;
    Serial.println(95478);
    if (Debug)
    {
      Serial.println("Cutter2 lowered"); 
    }
  }
  else if (!CUT2)
  {
